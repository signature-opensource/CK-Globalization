...
Invariant
```
The fallbacks of a culture are that path stripped at both ends - without the culture itself and without
the Invariant root: the ordered `CultureInfo.Parent` chain from the most specific parent down to the
neutral culture. `"pa-Guru-IN"` has `pa-guru, pa`; `"fr-FR"` has just `fr`; and a neutral culture like
`"fr"` has none, *"the Invariant never appears in this list"*. That last case is worth noting because
the chain above ends on `Invariant`, which makes it look like a fallback. It is not.

That is the rule for one culture. The `ExtendedCultureInfo` introduced below can also be a *pure* one,
defined by a list of cultures rather than derived from a single name - and there a neutral primary does
have fallbacks: `"fr,es"` falls back to `es`.
...
optimized to generate strings. If the format cannot be parsed successfully, a `GlobalizationAgent.TranslationFormatError` is emitted.
...
Calling `SetCachedTranslations` by hand is one way in. Two others exist and are worth knowing about:
`GlobalizationFileHelper` loads a folder of `.json`/`.jsonc` translation files at runtime, and
`GlobalizationTranslationInstaller` is a carrier whose body is *generated at build time* - a companion
engine walks each package's `Res/locales` folder, merges them per culture and emits the
`SetCachedTranslations` calls, so an application can start with its translations already registered and
no file to read.
...
## The ExtendedCultureInfo is an Ambient Service.
...
This immediate translation capability is available on the `UserMessage` helper, the `MCString.Create`
...
Two of the types above have documentation of their own, in their own folder:
[`FormattedString`](FormattedString/README.md) - what a captured interpolated string keeps, and why it
keeps the placeholder positions - and [`UserMessage`](UserMessage/README.md), with the
`UserMessageCollector` that turns a set of them into a validation context.
...
and `UserMessage` are serializable both with the simple and versioned simple binary serialization.
...
- `CK.Abstractions` for the DI markers - `ExtendedCultureInfo` is an `IAmbientAutoService` -
  `CK.ActivityMonitor.SimpleSender`, and `CK.PerfectEvent` for the agent's event surface.
